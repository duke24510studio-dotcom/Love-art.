...
既定の運用: `.github/workflows/video-pipeline.yml` が **毎朝06:00 JST (21:00 UTC) に scriptモードで1本** 生成する。
リポジトリシークレット `APP_URL` / `CRON_SECRET`（記事パイプラインと共通）を設定すれば有効になる。
良い台本だけを `/videos/[id]` で承認し、②音声 → ③画像 → ④組み立てを手動実行して週2〜3本を完成させる想定。
