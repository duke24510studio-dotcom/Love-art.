...
| direction | 収集元 | 出力言語 | 投稿先 | カテゴリ例 |
|-----------|--------|----------|--------|-----------|
| `en2ja`   | Medium (英語) の AI / ビジネス / ライフハック | 日本語 | note   | `ai`, `business`, `lifehack` |
| `ja2en`   | note・日本語ブログ の 禅 / 茶道 / 漫画 / 日本文化 | 英語 | Medium | `zen`, `tea`, `manga`, `culture` |
...
```
0 */3 * * *  →  POST /api/cron/pipeline  { "count": 1 }
```
