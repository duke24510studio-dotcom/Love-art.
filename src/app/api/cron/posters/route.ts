...
import { buildHokusaiPrompt, ensureHokusaiThemes, HOKUSAI_COLLECTION } from "@/lib/hokusai";
...
async function runPosterPipeline(count: number) {
  ...
  const generated: { id: string; themeEn: string }[] = [];
  const errors: string[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      const generation = await prisma.posterGeneration.create({
        data: {
          themeId: theme.id,
          prompt: buildHokusaiPrompt(theme),
          status: "prompted",
        },
      });
      const updated = await renderPosterImage(generation);
      generated.push({ id: updated.id, themeEn: theme.themeEn });
    } catch (err) {
...
async function handle(req: NextRequest, body: Record<string, unknown>) {
  ...
  try {
    const summary = await runPosterPipeline(count);
